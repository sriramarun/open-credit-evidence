...
| 5 | How it works | 3:30–4:15 | Six boxes, one breath each. Point at the green one: this is the thing under test. Point at box 5: one judge at run time, Nano, on our GPU; it grades readability and nothing else. Ultra appears in the chips as the teacher — it graded the training set once and is not called during a run. |
...
- **Why two judges?** There is one. Nano is the judge at run time, on-prem. Ultra graded 400 briefings once to make Nano's training set, and Luca checked a sample. The agreement table is how we know the small judge can be trusted.
