...
| Date | Change | By |
|---|---|---|
| 2026-09-16 | Assistant moved on-prem: Lightning NIM on the Codefest node `rtx-3se-05-36` (`scripts/cluster/serve_lightning.sh`, `docs/cluster.md`). Adapter takes `EVIDENCE_<ROLE>_BASE_URL`/`_MODEL`; `ChatResponse.endpoint` recorded. `notebooks/02_cluster_setup.ipynb` (newbie walkthrough, outputs committed). **Findings:** 2–3 s per call. Ten runs of APP000044 at temperature 0, seed 7: ten distinct outputs — vLLM MoE serving is not byte-deterministic, so reproducibility is defined as *same verdict across N runs, N recorded*; runner to do 3 passes per item. Stated DTI ratio wrong in 8 of 10 runs (39.6–44.7% vs 47.5% true) while `material_omission` passed all ten — `numeric_fidelity` is now first in the Week 2 check order, `decoy_citation` second (age band cited in ~6 of 10). | Sriram |
